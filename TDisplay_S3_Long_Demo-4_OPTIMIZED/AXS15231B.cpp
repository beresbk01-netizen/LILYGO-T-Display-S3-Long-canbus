...
// Rotation buffer in internal SRAM — fast stride writes, no PSRAM cache penalty
// 640*180*2 = 230400 bytes. Falls back to PSRAM if internal heap is full.
uint16_t* qBuffer = NULL; // allocated in axs15231_init()
...
{
    // Allocate rotation buffer in fast internal SRAM first, fall back to PSRAM
    qBuffer = (uint16_t*) heap_caps_malloc(230400, MALLOC_CAP_INTERNAL | MALLOC_CAP_8BIT);
    if (!qBuffer)
...
/* ─────────────────────────────────────────────────────────────────────
 *  lcd_PushFrame  —  rotate 640×180 landscape sprite → 180×640 portrait
 *  into qBuffer in one pass, then push via the same chunked SPI that
 *  the original lcd_PushColors_rotated_90 used (proven to work).
 *
 *  Speed gain over original: the rotation loop now writes qBuffer
 *  sequentially (j outer, i inner) which is the same access pattern
 *  as before — no regression. SPI chunk count is identical (8 × 14400).
 * ──────────────────────────────────────────────────────────────────── */
void IRAM_ATTR lcd_PushFrame(uint16_t *data)
{
    if (!data || !qBuffer) return;   // guard against failed malloc

    // Fast rotation: read source row-by-row (sequential PSRAM = cache friendly)
    // Write into internal SRAM qBuffer using pointer steps (no multiply)
    for (uint32_t y = 0; y < 180; y++) {
        const uint16_t *src = data + y * 640u;
        uint16_t *dst = qBuffer + (179u - y);
    ...
    // Portrait address window
    lcd_address_set(0, 0, 179, 639);

    // Proven chunked synchronous SPI push
    bool first_send = true;
    size_t len = 640UL * 180UL;
    uint16_t *q = qBuffer;

    TFT_CS_L;
    do {
        size_t chunk_size = (len > SEND_BUF_SIZE) ? SEND_BUF_SIZE : len;
        spi_transaction_ext_t t = {0};
        if (first_send) {
...
void lcd_PushFrame_sync(void) { /* nothing — synchronous, always complete */ }
