...
// Fast full-frame push with async DMA.
// Rotates sprite into internal-RAM buffer, then queues chunked SPI DMA.
// Returns immediately — DMA runs in background while CPU renders next frame.
void lcd_PushFrame(uint16_t *data);

// Block until the last lcd_PushFrame DMA transfer completes.
// Call this if you need to read qBuffer or before shutdown.
void lcd_PushFrame_sync(void);
